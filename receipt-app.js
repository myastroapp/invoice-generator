...
function render() {
  ...
  const meta = [];
  if ($("invNo").value) meta.push("Receipt: " + escHtml($("invNo").value));
  if ($("invDate").value) meta.push("Date: " + escHtml($("invDate").value));
  if ($("dueDate").value) meta.push("Paid: " + escHtml($("dueDate").value));
...
function wire() {
  ...
  const dateLabels = { invDate: "Receipt date", dueDate: "Payment date" };
