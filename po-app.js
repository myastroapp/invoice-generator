...
function render() {
  ...
  const meta = [];
  if ($("invNo").value) meta.push("PO #: " + escHtml($("invNo").value));
  if ($("invDate").value) meta.push("Order date: " + escHtml($("invDate").value));
  if ($("dueDate").value) meta.push("Deliver by: " + escHtml($("dueDate").value));
...
function wire() {
  ...
  const dateLabels = { invDate: "Order date", dueDate: "Delivery date" };
